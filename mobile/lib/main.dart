  ...
  Widget build(BuildContext context) {
    ...
    return ListView(padding: const EdgeInsets.fromLTRB(16, 8, 16, 36), children: [
      ...
      ...controller.plugins.plugins.map((plugin) => Card(child: Padding(padding: const EdgeInsets.only(bottom: 10), child: Column(children: [
        ListTile(leading: CircleAvatar(backgroundColor: plugin.accent, child: Text(plugin.icon)), title: Text(plugin.name, style: const TextStyle(fontWeight: FontWeight.w700)), subtitle: Text('${plugin.description}\n${plugin.author} · v${plugin.version}'), trailing: Row(mainAxisSize: MainAxisSize.min, children: [Switch(value: plugin.enabled, onChanged: (_) => controller.plugins.toggle(plugin.id)), if (!plugin.builtIn) IconButton(onPressed: () => controller.plugins.remove(plugin.id), icon: const Icon(Icons.delete_outline_rounded), tooltip: 'Remove plugin')])),
